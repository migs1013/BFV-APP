            ...
            switch(cmd)
            {
                ...
                case 3:  //FOR UPDATING PURPOSES
                    command = new MySqlCommand("SELECT `PART_NAME`,`TEST_NUMBER`,`TEST_NAME`,`TESTER_ID`,`TEST_STAGE`,`DATE_ENCOUNTERED`,`PRODUCT_OWNER`," +
                        "if(`STATUS` = 'VALID','CLOSED',`STATUS`) as `STATUS`,if(`STATUS` = 'FOR FURTHER INVESTIGATION' OR `STATUS` = 'FOR APPROVAL',datediff(curdate(),`DATE_ENCOUNTERED`),datediff(`DATE_APPROVED`,`DATE_ENCOUNTERED`)) as `CYCLE_TIME`,`ENDORSEMENT_NUMBER` FROM `hit`.`details` ORDER BY `ENDORSEMENT_NUMBER` DESC LIMIT 30", Connection.connect);
                    break;
                ...
                case 6:  // FOR SEARCH IN COMBO BOXES
                    command = new MySqlCommand(string.Format("Select `PART_NAME`,`TEST_NUMBER`,`TEST_NAME`,`TESTER_ID`,`TEST_STAGE`," +
                        "`DATE_ENCOUNTERED`,`PRODUCT_OWNER`,if(`STATUS` = 'VALID','CLOSED',`STATUS`) as `STATUS`,if(`STATUS` = 'FOR FURTHER INVESTIGATION' OR `STATUS` = 'FOR APPROVAL',datediff(curdate(),`DATE_ENCOUNTERED`),datediff(`DATE_APPROVED`,`DATE_ENCOUNTERED`)) as `CYCLE_TIME`,`ENDORSEMENT_NUMBER`" +
                        "FROM `hit`.`details` {0} ORDER BY `ENDORSEMENT_NUMBER` DESC LIMIT 30",FullTextCommand), Connection.connect);
                    break;
                case 7: //NEXT BUTTON
                    command = new MySqlCommand(string.Format("Select `PART_NAME`,`TEST_NUMBER`,`TEST_NAME`,`TESTER_ID`,`TEST_STAGE`,`PRODUCT_OWNER`," +
                ...
                case 17: // LOAD DATA IN DATAGRIDVIEW THAT HAS BEEN CLICK IN THE PARETO LIST
                    ...
                    command = new MySqlCommand(string.Format("SELECT `PRODUCT_OWNER`,`PART_NAME`,`TEST_NUMBER`,`TEST_NAME`,`TESTER_ID`,`TEST_STAGE`," +
                        "`DATE_ENCOUNTERED`,if(`STATUS` = 'VALID','CLOSED',`STATUS`) as `STATUS`,if(`STATUS` = 'FOR FURTHER INVESTIGATION' OR `STATUS` = 'FOR APPROVAL',datediff(curdate(),`DATE_ENCOUNTERED`),datediff(`DATE_APPROVED`,`DATE_ENCOUNTERED`)) as `CYCLE_TIME`,`ENDORSEMENT_NUMBER` FROM `DETAILS` " +
